...
runner MUST first drop the collection. The test runner must also drop the collections `_enxcol.<collectionName>.esc`,
`_enxcol.<collectionName>.ecoc`. coll If a `createOptions` document is present, the test runner MUST execute a `create`
